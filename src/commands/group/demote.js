...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('demote')
    .setDescription('Demote a member in your group')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    .addUserOption((opt) => opt.setName('user').setDescription('The Discord user to demote').setRequired(true)),
  cooldown: 5,
  ...
  async execute(interaction, client) {
    ...
    try {
      const result = await nobloxService.demoteUser(Number(guildConfig.roblox.groupId), Number(userDoc.robloxUserId));
      await interaction.editReply({
        embeds: [
          embed.success('⬇️ User Demoted', '')
            .setDescription(null)
            .addFields(
      ...
      await interaction.editReply({ embeds: [embed.error('Demote Failed', err.message)] });
    }
  },
};
